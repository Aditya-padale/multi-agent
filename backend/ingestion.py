...
runs instantly and is plenty good for a research-agent demo. Swap in real
embeddings later if you want semantic (not just lexical) retrieval.
"""
import io
import re
import uuid
...
class DocStore:
    ...
    def add_document(self, source: str, doc_type: str, text: str, chunk_size: int = 900, overlap: int = 150):
        ...
        while start < len(text):
            ...
            self.chunks.append(Chunk(id=str(uuid.uuid4())[:8], source=source, doc_type=doc_type, text=piece))
...
def ingest_file(store: DocStore, filename: str, file_bytes: bytes):
...
def ingest_webpage(store: DocStore, url: str):
