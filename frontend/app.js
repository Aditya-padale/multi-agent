...
// Keep debate data globally so round tabs can re-render
...
async function refreshLeaderboard() {
  ...
  leaderboardEl.innerHTML = data.leaderboard
    .map((row, i) => {
      ...
      const label = data.personas[row.agent] || row.agent;
      return `
...
el("resetDocsBtn").addEventListener("click", async () => {
  await fetch(`${API}/api/reset-documents`, { method: "POST" });
  await refreshSources();
  setStatus("evidence cleared");
});

el("resetPolicyBtn").addEventListener("click", async () => {
  await fetch(`${API}/api/reset-policy`, { method: "POST" });
...
async function runBattle() {
  ...
  try {
    const res = await fetch(`${API}/api/ask`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ question, num_agents: numAgents, debate_rounds: debateRounds }),
    });
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.detail || res.statusText);
